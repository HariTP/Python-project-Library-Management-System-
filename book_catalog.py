...
def book_data():                   ## Main function to be called in Main Menu
    win_ad=Tk()
    win_ad.title("Book Data")
    win_ad.configure(bg="light blue")
    win_ad.geometry('750x250+290+200')

    Label(win_ad,text="SEARCH BOOK DATA",bg='light blue',borderwidth=5,relief="raised",fg="dark blue",font=('Century Schoolbook L',25,'bold')).place(x=180,y=10)
    ...
    Label(win_ad,text="Name",bg='light blue',font=('Century Schoolbook L',20,'bold')).place(x=50,y=90)
    ...
    def search_book():
        if (e1.get()=="" or e1.get() is None):
            ...
            name=e1.get()       
            str1="select * from books where name like '{}%'".format(name)
            ...
            if data==[]:
                ...
                tab.pack()
                tab.heading(1,text="Book No")
                tab.heading(2,text="Name")
                tab.heading(3,text="Author")
                tab.heading(4,text="Status")
                for i in data:
                    if i[3]==0:
                        j=(i[0],i[1],i[2],"Available")
                    elif i[3]==1:
                        j=(i[0],i[1],i[2],"Not Available")
                    tab.insert('','end',values=j)
                bt3=Button(win_dat,text="Close",bd=3,bg="light green",width=8,height=1,command=win_dat.destroy).place(x=380,y=140)    

    bt1=Button(win_ad,text="OK",bd=3,bg="light green",width=8,height=2,command=search_book).place(x=230,y=180)
